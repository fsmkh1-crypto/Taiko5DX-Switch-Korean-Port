...
- V272–V284: `docs/VALIDATION_LEDGER_KO_FULL_SENTENCE_GRAMMAR_FLATTENING_V1_DESIGN.txt`
...
- current runtime byte-validation/copy counterpart census: `docs/RUNTIME_BYTE_VALIDATION_COPY_CONSUMER_CENSUS.txt`
...
Later narrower-scope corrections take precedence over older semantic overclaims. `WRITE_SAFE` exists only where a dedicated authorization artifact explicitly grants it. Mapping V240–V245 granted static write authority to the exact four guarded actions; V246–V251 recanonicalized the helper implementation as reproducible HELPER_TEXT_V2 without changing action cardinality; V252–V257 materialized the exact four-action diagnostic; V258–V260 closes that realization's tested forward/reverse runtime route on Eden Android. V261–V264 materializes the current source-complete portability/action planning population without declaring a final global Action Ledger cardinality or adding write authority. V265–V270 closes the runtime_byte_validation/copy counterpart census at analysis level: renderer decode compatibility is insufficient for native-equivalent closure because an upstream Japanese halfwidth normalization/copy owner overlaps compact Korean byte values; counterpart is found, native equivalence is rejected, P3 runtime-port realization remains a planning candidate pending route binding, and no write authority is added. V272–V276 materializes the Korean full-sentence grammar-flattening design direction and exact-PC-runtime provenance limitation. V277 closes the whole-family census; V278 records the initial caller matrix/control invariants; V279 supersedes automatic treatment of the 913 structurally-direct callers after speech-style ownership review; V280 records the composition/morphology lower-bound risk census; V281 fixes the reviewed migration contract and selects the 567 caller-style-branch population as the next read-only detailed matrix. V282 partitions that 567 population into 253 root-style-only, 207 root-style-plus-secondary, 64 nested-style, and 43 pre-style-grammar-mixed callers and rejects caller `0x6A` presence as complete tone ownership. V283 closes the 460 root-style-owner tone-obligation matrix: none of the 460 callers directly covers all nested formatter relationship/register predicates, so all 460 require explicit tone-collapse review before text migration. V284 establishes the first reusable Korean V1 tone policy from the 253 root-style-only callers, classifies all 34 used grammar entry families into register-normalize, lexical/mood rewrite, zero-output structural exception, and honorific-role critical classes, and selects the pure 125 register-normalization callers as the next READ_ONLY wording-design tranche. None of V272–V284 adds gameplay mutation, build, or WRITE_SAFE authority. Physical Nintendo Switch validation and unrelated renderer/font integration remain separate. FZ001 remains immutable; current effective source state is composed from frozen baseline plus exact later canonical overlays within each overlay's proven axis/scope.