...
| V031 | W0 v0.2m is stable on the tested early-game route and corrects the previously odd normal two-byte `케/자` rendering in `나야 스케자에몬`, but the compact page-63 `BD=쓰` in `마쓰다이라 모토야스` remains visually absent. | VERIFIED-RUNTIME | User tested W0 alone on 2026-09-11 and supplied screenshots. `나야 스케자에몬` now renders `케/자` normally. The Matsudaira name still appears effectively as `마 다이라모토야스`; therefore the six W0 thresholds are useful but not sufficient for compact single-byte rendering. Artifact SHA-256 `e453d5958793748ebf841f295ef4005a9a612fdbe643439fe9c2a2c0183ea2ad`. |
| V032 | A seventh text-render/layout decision at mapped `0x44D9D0` treats every code `<0x100` as width 8 immediately before the glyph construction/draw path; it is a high-confidence missed counterpart for compact `A1+` Korean glyph rendering. | VERIFIED | Disassembly around `0x44D9C4..0x44DA30`: `and w8,w26,#0xffff; cmp w8,#0x100; b.hs 0x44D9E0; mov w19,#8`, followed by the alternate table-width path and `bl 0x44E400`. W1 changes only this compare to `cmp w8,#0xA1` on top of W0. Artifact `W1_Taiko5DX_KR_DBG_FONTWIDTH_RENDER_v0.2n.zip`, SHA-256 `26fcd1434561b2e02d797079d6d996e5becfe66808e763d0b309ee1db10d44d1`; runtime pending. |
