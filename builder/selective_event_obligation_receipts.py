...
class _Evaluation:
    ...
    def check_root(self, oid):
        def fn():
            ...
            actual = self.slice(b.placement, 'TAI5MSG:FINAL')
            require(len(actual) == r['target_length'] and sha256(actual) == r['target_sha256'],
                    'ROOT_FINAL_VALUE_IDENTITY_MISMATCH', oid)
            return 'ROOT_VALUE_VERIFIED_LOCAL', ['ACTUAL_FINAL_VALUE_SIZE_SHA256','EXACT_BLOCK_LOCAL_LOCATOR'], [], {
                'root':r['owner'],'sha256':sha256(actual),'size':len(actual),'final_range':[b.placement.span.start,b.placement.span.end],
                'locator_authority':b.locator_proof.document,'claim':'VALUE_ONLY; locator semantics and TAI5MSG-wide acceptance remain external'}
