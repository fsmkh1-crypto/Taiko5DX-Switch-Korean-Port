...
- `SELECTIVE_PROJECT_STATE.md` — 이 선택형 트랙의 현재 상태와 다음 단계
...
초기 설계와 1차 자산 이관 완료 단계다.

다음 실행 범위는 `SELECTIVE_KO_SOURCE_CLASSIFICATION_READ_ONLY`이며, 기존 PC Korean corpus와 Switch owner 자료에서 실제로 필요한 설명/이벤트/안전 대사 후보를 추출·분류한다. 이 단계 전에는 새 builder나 release artifact를 만들지 않는다.
